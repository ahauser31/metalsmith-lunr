...
function createDocumentIndex(opts, file, path){
  ...
  for (field in opts.fields){
    if(field === 'contents'){
      ...
      if (typeof file[field] !== 'undefined') index[field] = sanitizeHtml(file[field], opts.sanitizeHtml);
    }
  }
  // console.log(index);
  return index;
...
function setDefaultOptions(opts){
	...
	opts.stripHtml = (typeof opts.stripHtml === 'undefined') ? true : opts.stripHtml;
	opts.sanitizeHtml = opts.stripHtml ? {allowedTags: [], allowedAttributes:[]} : {};
    return opts;
}
